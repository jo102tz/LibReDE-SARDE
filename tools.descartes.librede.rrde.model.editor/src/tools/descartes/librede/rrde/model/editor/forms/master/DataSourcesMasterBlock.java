...
import tools.descartes.librede.configuration.editor.forms.master.AbstractMasterBlockWithButtons;
...
public class DataSourcesMasterBlock
		extends tools.descartes.librede.rrde.model.editor.forms.master.AbstractMasterBlockWithButtons
	 ...
	 */
	public DataSourcesMasterBlock(AdapterFactoryEditingDomain domain, LifeCycleConfiguration model) {
		super(domain, model);
	}
		...
		if (inputData != null) {
			for (Object r : results) {
				...
				if (inputData.getInput() == null) {
					...
					inpSpec.getDataSources().add(source);
					cmd = SetCommand.create(domain, inputData, OptimizationPackage.Literals.INPUT_DATA__INPUT, inpSpec);
				} else {
					cmd = AddCommand.create(domain, inputData.getInput(),
							ConfigurationPackage.Literals.INPUT_SPECIFICATION__DATA_SOURCES, source);
				}
	...
	public IDetailsPage getPage(Object key) {
		if (key instanceof String) {
			return new ParametersDetailsPage(page, domain, "Data Source Configuration",
					ConfigurationPackage.Literals.DATA_SOURCE_CONFIGURATION, (String) key,
