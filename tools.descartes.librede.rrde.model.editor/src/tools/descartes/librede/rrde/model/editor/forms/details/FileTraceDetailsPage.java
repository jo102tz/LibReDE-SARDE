...
public class FileTraceDetailsPage extends AbstractDetailsPage {
	...
	private InputData inputSelection;
	 ...
	 */
	public FileTraceDetailsPage(AbstractLifecycleConfigurationFormPage page, AdapterFactoryEditingDomain domain,
	...
	public void createContents(Composite parent) {
		...
		IObservableList metrics = EMFEditProperties.list(domain, MetricsPackage.Literals.METRICS_REPOSITORY__METRICS)
				.observe(Registry.INSTANCE.getMetricsRepository());
		comboMetricViewer = createComboBoxViewer(composite, toolkit, metrics);

		// make this a post selection listener, as changes should only
		// propagated to the
		...
		Composite fileEditorComposite = toolkit.createComposite(composite, SWT.NONE);
		...
		Button btnBrowse = toolkit.createButton(fileEditorComposite, "Browse...", SWT.PUSH);
		...
		Label lblDataSource = toolkit.createLabel(composite, "Data Source:", SWT.NONE);
		comboDataSourceViewer = createComboBoxViewer(composite, toolkit,
				EMFEditProperties
						.list(domain,
								FeaturePath.fromList(
										OptimizationPackage.Literals.INPUT_DATA__INPUT,
										ConfigurationPackage.Literals.INPUT_SPECIFICATION__DATA_SOURCES))
						.observe(inputSelection));
		...
		comboAggregationViewer = createComboBoxViewer(composite, toolkit,
				new WritableList(Aggregation.VALUES, Aggregation.class));

		Label lblInterval = toolkit.createLabel(composite, "Interval:", SWT.NONE);
		...
		comboIntervalUnitViewer = TimeUnitSpinnerBuilder.createTimeUnitControl(toolkit, page.getAdapterFactory(),
				intervalEditorComposite);

		Label lblMapping = toolkit.createLabel(composite, "Mapping:", SWT.NONE);
		...
		mappingTableViewer = new TableViewer(mappingTableComposite,
		...
		entityViewerColumn.setEditingSupport(AbstractEstimationConfigurationFormPage.EObjectEditingSupport.create(
				mappingTableViewer, domain, ConfigurationPackage.Literals.OBSERVATION_TO_ENTITY_MAPPING__ENTITY));
		...
		indexViewerColumn.setEditingSupport(AbstractEstimationConfigurationFormPage.EObjectEditingSupport.create(
				mappingTableViewer, domain, ConfigurationPackage.Literals.TRACE_TO_ENTITY_MAPPING__TRACE_COLUMN));

		mappingTableViewer.setContentProvider(new AdapterFactoryContentProvider(page.getAdapterFactory()));
		mappingTableViewer.setLabelProvider(new AdapterFactoryLabelProvider(page.getAdapterFactory()));

		mappingTableViewer.getControl().addMouseListener(new MouseAdapter() {
			@Override
			public void mouseDown(MouseEvent e) {
				page.getLifecycleEditor().setCurrentViewer(mappingTableViewer);
			}
		});
		page.getLifecycleEditor().createContextMenuFor(mappingTableViewer);
	...
	private void createBindings() {
		...
		detailBindingContext.bindValue(WidgetProperties.text(SWT.Modify).observe(txtFilePath), EMFEditProperties
				.value(domain, ConfigurationPackage.Literals.FILE_TRACE_CONFIGURATION__FILE).observe(input));
		detailBindingContext.bindValue(ViewerProperties.singleSelection().observe(comboMetricViewer),
				EMFEditProperties.value(domain, ConfigurationPackage.Literals.OBSERVATION__METRIC).observe(input));
		detailBindingContext.bindValue(ViewerProperties.singleSelection().observe(comboUnitViewer), EMFEditProperties
				.value(domain, ConfigurationPackage.Literals.TRACE_CONFIGURATION__UNIT).observe(input));
		detailBindingContext.bindValue(ViewerProperties.singleSelection().observe(comboAggregationViewer),
				EMFEditProperties.value(domain, ConfigurationPackage.Literals.OBSERVATION__AGGREGATION).observe(input));
		detailBindingContext.bindValue(WidgetProperties.selection().observe(spnIntervalValue),
				EMFEditProperties.value(domain,
						FeaturePath.fromList(ConfigurationPackage.Literals.TRACE_CONFIGURATION__INTERVAL,
		...
		detailBindingContext
				.bindValue(
						ViewerProperties
								.singleSelection().observe(comboIntervalUnitViewer),
						EMFEditProperties.value(domain,
								FeaturePath.fromList(ConfigurationPackage.Literals.TRACE_CONFIGURATION__INTERVAL,
										UnitsPackage.Literals.QUANTITY__UNIT))
								.observe(input));
		detailBindingContext
				.bindValue(ViewerProperties.singleSelection().observe(comboDataSourceViewer),
						EMFEditProperties
								.value(domain,
										FeaturePath.fromList(
	...
	public void selectionChanged(IFormPart part, ISelection selection) {
		inputSelection = SelectionProvider.INSTANCE().getSelectedInputData();
		if (inputSelection != null) {
			...
			if (structuredSelection.size() == 1) {
				...
				IObservableList units = EMFEditProperties
						.list(domain, FeaturePath.fromList(ConfigurationPackage.Literals.OBSERVATION__METRIC,
								MetricsPackage.Literals.METRIC__DIMENSION, UnitsPackage.Literals.DIMENSION__UNITS))
						.observe(input);
				comboUnitViewer.setInput(units);
				
				IObservableList dataSrc = EMFEditProperties
				.list(domain,
						FeaturePath.fromList(
								OptimizationPackage.Literals.INPUT_DATA__INPUT,
								ConfigurationPackage.Literals.INPUT_SPECIFICATION__DATA_SOURCES))
				.observe(inputSelection);				
				comboDataSourceViewer.setInput(dataSrc);
	...
	private void handleAddMapping() {
		if (inputSelection != null) {
			...
			// The label provider will then indicate to the user that he needs
			// to
			// select one manually.
			
			if (inputSelection.getWorkloadDescription() == null) {
				WorkloadDescription desc = ConfigurationFactory.eINSTANCE.createWorkloadDescription();
				Command cmd = SetCommand.create(domain, inputSelection, OptimizationPackage.Literals.INPUT_DATA__WORKLOAD_DESCRIPTION, desc);
				domain.getCommandStack().execute(cmd);
			}
			for (Resource r : inputSelection.getWorkloadDescription().getResources()) {
			...
			if (mapping.getEntity() == null) {
				for (Service s : inputSelection.getWorkloadDescription().getServices()) {
			...
			Command cmd = AddCommand.create(domain, input, ConfigurationPackage.Literals.TRACE_CONFIGURATION__MAPPINGS,
					mapping);
			domain.getCommandStack().execute(cmd);
		}
	}
