...
public class Evaluate {
	...
	public void test() {
		...
		configs = Discovery.createConfigurations(recommendation.getTrainingData(), librede.getEstimation(), validator);
...
//     recommendation);
	...
	private void validateNothing() {
		...
		for (LibredeConfiguration c : configs) {
			Util.setEstimationSpec(c, EcoreUtil.copy(librede.getEstimation()));
		}
	...
	private void validateOptimizationAndRecommendation(LibredeConfiguration librede,
		...
		vali = new TestSetValidator(configs, validator);
		for (LibredeConfiguration c : configs) {
			Discovery.fixTimeStamps(c, null);
		}
		Assert.assertNotEquals(vali.getTestset().size(), 0);
...
//		RecommendationAlgorithmSpecifier[] algos = { warmup, tree, nn, svm, rf };
	...
	private void validateOptimizers(LibredeConfiguration libconf, OptimizationConfiguration conf) {
		...
		map = evaluateApproachesWithConfiguration(newRunCalls, newRunCalls.iterator().next().getAlgorithm(), conf,
				libconf, estimationList, newRunCalls.iterator().next().getSettings().getParametersToOptimize());
		printSolutions(file, estimationList, map);
	...
	private void validateAllOptimizersAutomatically(LibredeConfiguration libconf, OptimizationConfiguration conf) {
		...
		String folder = newRunCalls.iterator().next().getTrainingData().iterator().next().getRootFolder();
		for (RunCall call : newRunCalls) {
			for (InputData data : call.getTrainingData()) {
				data.setRootFolder(validationfolder);
			}
		}
		map = evaluateApproachesWithConfiguration(newRunCalls, spec, conf, libconf, estimationList, params);
		printSolutions(file, estimationList, map);
		// restore
		for (RunCall call : newRunCalls) {
			for (InputData data : call.getTrainingData()) {
		...
		map = evaluateApproachesWithConfiguration(newRunCalls, ipo, conf, libconf, estimationList, params);
		printSolutions(file, estimationList, map);
	...
	private HashMap<EstimationApproachConfiguration, StatisticsSummary> evaluateApproachesWithConfiguration(
			...
			ArrayList<EstimationApproachConfiguration> estimationList, List<IOptimizableParameter> params) {
		...
		for (RunCall run : newRunCalls) {
			...
			vali = new TestSetValidator(configs, validator);
			for (LibredeConfiguration c : configs) {
				Util.setEstimationSpec(c, EcoreUtil.copy(run.getEstimation()));
			}
			Assert.assertNotEquals(vali.getTestset().size(), 0);
			vali.calculateInitialErrors();
	...
	private void printSolutions(FileExporter file, ArrayList<EstimationApproachConfiguration> estimations,
		...
		for (IOptimizableParameter parameter : map.values().iterator().next().getParameters().keySet()) {
			file.writeString("");
